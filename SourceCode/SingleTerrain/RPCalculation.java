...
public class RPCalculation 
{
	...
	float [][] network; // network, in 2d square matrix
	...
	public RPCalculation (float [][] argNet)
	{
		numNode = argNet.length;
	...
	public RPCalculation (float [][] argNet, float [] argNodeWeight)
	{
		numNode = argNet.length;
		RP = new float [numNode];
		RP0 = new float [numNode];
		...
		for (int t = 0; t < maxIter; t++)
		{
			...
			for (int i = 0; i < numNode; i++)
			{
				...
				for (int j = 0; j < numNode; j++)
				{
					...
					{
						RP[i] += sigma * tempRP[j] * network[j][i] / nodeWeightDeg[j];
					}
				}
			}
		}
